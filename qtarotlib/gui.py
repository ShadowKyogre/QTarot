...
class QTarot(QtGui.QMainWindow):
    ...
    def saveReadingAsHTML(self, filename):
        ...
        f.write(template.format(cards=cards, deck=deck, layout=layout, \
                   reading_px=reading_pxh, layout_credits=layout_credits, \
    ...
    def newReading(self, item=None, neg=None, skin=None, deck=None, 
                   ask_for_deck=False):
        ...
        if ask_for_deck:
            deck, ok = QtGui.QInputDialog.getItem(self, "Generate new reading",
                           "Deck definition to use:", list(qtrcfg.deck_defs.keys()),
                           0, False)
            if ok and deck:
                deck = str(deck)
            else:
        ...
        for s in def_data.source[:]:
            if s.text:
                if urlparse(s.text).scheme:
                    sources.append(("<a href=\"{s.text}\">{s.text}</a>").format(s=s))
                else:
    ...
    def generateCardText(self, card, reverse=None, purpose=None, newfp=None, skin=''):
        ...
        reading_specific = ("<br />\n\t\tCurrent status: {status}<br />"
                            "\n\t\tPurpose in layout: {purp}") \
                           if reverse is not None and purpose is not None \
                           else ""
        if newfp:
        ...
        revtext = markdown(card.meaning.reversed.text) if card.meaning.reversed.text \
                  else "Cannot be reversed"
        result = template.format(fn=fn, name=card.fullname(), n=card.number, \
                             suit=card.getparent().get('name'), \
    ...
    def getTableName(self):
        text = QtGui.QFileDialog.getOpenFileName(self, caption="Save Current Reading",
                   ...
                   filter="Images ({})".format(' '.join(formats))
               )
        if text > "":
...
def main():
    ...
    parser = argparse.ArgumentParser(prog='qtarot', 
                 description="A simple tarot fortune teller"
    )

    parser.add_argument('-l', '--layout', help='The layout to use.', 
        default=qtrcfg.default_layout, choices=list(qtrcfg.layouts.keys())
    )
    parser.add_argument('-t', '--table', help='File to use as table', default=qtrcfg.table)
    parser.add_argument('-n', '--negativity', help='How often cards are reversed', 
        default=qtrcfg.negativity, type=float
    )
    parser.add_argument('-o', '--output', help='Save the reading to this file', default=None)
    parser.add_argument('-d', '--deck', help='Deck definition to use', 
        default=qtrcfg.deck_def, choices=list(qtrcfg.deck_defs.keys())
    )
    parser.add_argument('-s', '--skin', 
        help="Deck skin to use (valid values depend on deck definition)", 
        default=qtrcfg.deck_skin
    )
    ...
    if args.deck != qtrcfg.deck_def or  args.skin != qtrcfg.deck_skin:
        if args.skin not in qtrcfg.deck_defs[args.deck]['skins']:
            print(("Invalid skin \"{}\" for {}!\nValid skins: {}").format(
                         args.skin, args.deck, qtrcfg.deck_defs[args.deck]['skins'])
            )
            exit(1)
